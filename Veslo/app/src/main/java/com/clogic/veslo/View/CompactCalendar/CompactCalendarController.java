...
class CompactCalendarController {
    ...
    private int daysNameBackgroundColor;
    ...
    private void loadAttributes(AttributeSet attrs, Context context) {
        if (attrs != null && context != null) {
            ...
            try {
                ...
                daysNameBackgroundColor = typedArray.getColor(R.styleable.CompactCalendarView_compactCalendarDaysNameBackgroundColor, calendarBackgroundColor);
    ...
    void onMeasure(int width, int height, int paddingRight, int paddingLeft) {
        ...
        smallIndicatorRadius = widthPerDay/10;
        ...
        bigCircleIndicatorRadius = widthPerDay/3;
    }
    ...
    private void drawCalendarBackground(Canvas canvas) {
        ...
        dayPaint.setColor(daysNameBackgroundColor);
        canvas.drawRect(0, 0, width, heightPerDay - heightPerDay/4, dayPaint);
        dayPaint.setStyle(Paint.Style.STROKE);
        dayPaint.setStrokeWidth(2);
        dayPaint.setColor(context.getResources().getColor(R.color.main_transparent_white));
        canvas.drawLine(0, heightPerDay - heightPerDay/4, width, heightPerDay - heightPerDay/4, dayPaint);
        dayPaint.setStrokeWidth(0);
        dayPaint.setColor(calenderTextColor);
    }

    void drawEvents(Canvas canvas, Calendar currentMonthToDrawCalender, int offset) {
        drawEvents(canvas, currentMonthToDrawCalender, offset, 0);
    }

    void drawEvents(Canvas canvas, Calendar currentMonthToDrawCalender, int offset, int additionalPaddingTop) {
        ...
        if (uniqCalendarDayEvents != null) {
            for (int i = 0; i < uniqCalendarDayEvents.size(); i++) {
                ...
                if (showSmallIndicator) {
                    ...
                    drawSmallIndicatorCircle(canvas, xPosition, yPosition + additionalPaddingTop - additionalPaddingTop/4, event.getColor());
                } else {
                    drawCircle(canvas, xPosition, yPosition + additionalPaddingTop, event.getColor());
                }
            }
        }
    }
    ...
    void drawMonth(Canvas canvas, Calendar monthToDrawCalender, int offset) {
        dayPaint.setColor(calenderTextColor);
        int month = monthToDrawCalender.get(Calendar.MONTH) + 1;
        canvas.drawText(String.valueOf(month) + "월",
                widthPerDay * 3 + offset + paddingWidth + paddingLeft + accumulatedScrollOffset.x - paddingRight,
                heightPerDay + heightPerDay/4,
                dayPaint);

        Rect rect = new Rect();
        dayPaint.getTextBounds(String.valueOf(month), 0, 1, rect);
        int additionalPaddingTop = heightPerDay/2 + rect.height();

        drawEvents(canvas, monthToDrawCalender, offset, additionalPaddingTop);
        ...
        for(int i=0; i<7; i++) {
            ...
            float xPosition = widthPerDay * i + paddingWidth + paddingLeft + accumulatedScrollOffset.x + offset - paddingRight;
            if (shouldDrawDaysHeader) {
                dayPaint.setTypeface(Typeface.DEFAULT_BOLD);
                canvas.drawText(dayColumnNames[i], xPosition, paddingHeight, dayPaint);
        ...
        for (int dayColumn = 0, dayRow = 0; dayColumn <= 6; dayRow++) {
            ...
            Logger.i("day : " + day);
            float yPosition = dayRow * heightPerDay + paddingHeight + additionalPaddingTop - heightPerDay/2;
